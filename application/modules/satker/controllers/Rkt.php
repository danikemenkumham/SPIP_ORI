...
class Rkt extends MX_Controller {
    ...
    public function detail($idrkt,$id_satker=NULL){
        ...
        $data['waktu'] = $this->satker->get_target_waktu();
    ...
    public function add_catatan(){
        ...
        $catatan = $this->satker->get_catatan($this->input->post('detail'), $this->input->post('waktu'));
        //send notif
        if($this->session->userdata('role') == 5){
            $notif = array(
    ...
    public function get_catatan($detail, $waktu){
        return $this->satker->get_catatan($detail, $waktu);
    }
