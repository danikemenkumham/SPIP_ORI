...
class Wbk extends MX_Controller {
    ...
    public function count_lke_area($id_area, $id_satker){
     ...
     function paper($id_satker){
        ...
        $data['wbk'] = $this->satker->get_cetak($id_satker, $this->session->userdata('tahun'));
        //$this->load->view('wbk/paper', $data);
        
        $this->load->library('pdf');   
        $this->pdf->load_view('wbk/paper', $data);
        
        $this->pdf->setPaper('A4', 'landscape');
        $this->pdf->render();
        $satker_name = modules::run('satker/pmprb/get_satker', $id_satker);
        $file_name = 'LKE_WBK/WBBM_'.$satker_name.'_'.$this->session->userdata('tahun');
     ...
     function print_paper($id_satker){
        
        $data['wbk'] = $this->satker->get_cetak($id_satker, $this->session->userdata('tahun'));
        $this->load->view('wbk/print_paper', $data);
    ...
    public function count_wbk_cetak($id_wbk, $id_satker){
        $coun_all = $this->satker->total_lke($id_wbk, $id_satker); 
        $coun_lengkap = $this->satker->total_lengkap_lke($id_wbk, $id_satker); 
        if($coun_lengkap == 0){
             $presentase = 0;
      ...
      function wbk_reupload(){
        $reupload= $this->satker->check_satker_reupload();
      echo   $reupload->row()->status;
